...
import requests
import functools
from config import config, update_meli_tokens

BASE_URL = "https://api.mercadolibre.com"

def exchange_code_for_tokens(code):
    url = f"{BASE_URL}/oauth/token"
    payload = {'grant_type': 'authorization_code', 'client_id': config.MELI_APP_ID, 'client_secret': config.MELI_SECRET_KEY, 'code': code, 'redirect_uri': config.REDIRECT_URI}
...
def refresh_access_token():
    ...
    url = f"{BASE_URL}/oauth/token"
    payload = {'grant_type': 'refresh_token', 'client_id': config.MELI_APP_ID, 'client_secret': config.MELI_SECRET_KEY, 'refresh_token': config.MELI_REFRESH_TOKEN}
    headers = {'accept': 'application/json', 'content-type': 'application/x-www-form-urlencoded'}
    response = requests.post(url, headers=headers, data=payload, timeout=15)
    response.raise_for_status()
    new_tokens = response.json()
    update_meli_tokens(new_tokens['access_token'], new_tokens['refresh_token'])
    return True
